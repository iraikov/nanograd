;;; nanograd/examples/am-regression-ssa.scm
;;;
;;; Array-morphisms regression example using the SSA backend.
;;; Same task as am-regression.scm: y = sin(x1) + cos(x2) + x3^2 - 0.5*x4
;;;
;;; Differences from am-regression.scm:
;;;   - Single morphism context instead of two (ctx-fwd / ctx-bwd).
;;;   - am-training-step/ssa compiles the joint forward+backward to SSA once
;;;     on the first call and replays it on every subsequent call.
;;;   - tgt-lt is passed as an extra-input so its SSA constant is updated
;;;     each step (the fix for the target-constant bug).
;;;   - No explicit am-zero-grad! / am-reset-context! calls; handled inside
...
(printf "\n")
(printf "  NanoGrad AM Regression Example (SSA backend)\n")
(printf "  y = sin(x1) + cos(x2) + x3^2 - 0.5*x4\n")
(printf "  (array-morphisms SSA fused forward+backward)\n")
(printf "\n")
...
;; Model: 4 -> 64 -> 32 -> 16 -> 1 with ReLU hidden layers
(define mb-size 32)
...
;; Optimizer and single SSA context
...
(let epoch-loop ((epoch 1))
  (when (<= epoch num-epochs)
    (let* ((shuffled    (shuffle train-data))
      ...
      ;; batch has the same shape (required for SSA replay).
      (let batch-loop ((rem shuffled))
        (when (>= (length rem) mb-size)
          (let* ((batch   (take rem mb-size))
                 ...
                 (x-lt    (car bp))
                 (tgt-lt  (cdr bp))
                 ;; loss-fn captures tgt-lt for the SSA trace on the first call.
                 ;; tgt-lt is also passed as an extra-input so its SSA constant
                 ;; is updated on every replay step.
                 (loss-fn (lambda (pred-lt) (am-mse-loss pred-lt tgt-lt)))
                 (loss-lt (am-training-step/ssa ctx opt model loss-fn x-lt tgt-lt))
...
;;; ============================================================
;;; Context stats: buffer reuse summary (single SSA context)
;;; ============================================================
...
(let* ((total-steps (* num-epochs (quotient (length train-data) mb-size)))
  ...
  (printf "SSA context buffer reuse (~A total steps):\n" total-steps)
